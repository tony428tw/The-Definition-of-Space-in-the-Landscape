...
export default function SectionView({ walls=[], stage=3, label, a, b, trees, direction, depth, treeHeight, trunkHeight, dbhCm, dimensions, download }: {
  walls?:Wall[];stage?:number;label: string; a: SectionPoint; b: SectionPoint; trees: SectionPoint[]; direction: number; depth: number; treeHeight: number; trunkHeight: number; dbhCm:number; dimensions: boolean;
  ...
  const crownHeight=Math.max(.1,treeHeight-trunkHeight),trunkWidth=dbhCm/100;
  const scale = Math.min(660 / model.length, 300 / treeHeight);
    ...
    {viewport.controls}<svg ref={ref} viewBox={viewport.viewBox} xmlns="http://www.w3.org/2000/svg" role="img" aria-label={`${label}–${label}′ 基地剖立面圖`} style={{width:"100%",background:"white"}}>
      ...
      <g fontFamily="sans-serif" fill="#263b30" fontSize="16">
        ...
        <g clipPath="url(#section-extent)">{wallDrawing(false)}{model.trees.map(t => {
          ...
          const top=ground-treeHeight*scale, bottom=ground-trunkHeight*scale;
          ...
          return <g key={t.id} fill={t.cut ? "#dae6dc" : "#f0f4f0"} stroke={t.cut ? "#264c34" : "#94a79a"} strokeWidth={t.cut ? 2.5 : 1}>
            {(Math.abs(t.distance) < trunkWidth/2 || !t.cut) && <rect x={x-trunkWidth/2*scale} y={ground-trunkHeight*scale-crownHeight*scale/2} width={trunkWidth*scale} height={(trunkHeight+crownHeight/2)*scale} fill={t.cut ? "#52644e" : "#d5ddd5"}/>}
        ...
        {dimensions && <g stroke="#435b4b" strokeWidth="1" fill="#263b30">
          {[{x:left+width+32,z0:0,z1:trunkHeight,label:`枝下高 ${trunkHeight.toFixed(1)} m`},{x:left+width+32,z0:trunkHeight,z1:treeHeight,label:`冠高 ${crownHeight.toFixed(1)} m`},{x:left-48,z0:0,z1:treeHeight,label:`樹高 ${treeHeight.toFixed(1)} m`}].map((d,i)=>{
          ...
          {[0,trunkHeight,treeHeight].map(z=><path key={z} d={`M${left} ${ground-z*scale}H${left+width+40}`} strokeDasharray="4 5" strokeOpacity=".3"/>)}
        </g>}
        {dimensions && <><path d={`M${left} 432v12m0 -6h${width}m0 -6v12`} fill="none" stroke="#263b30"/><text x="500" y="460" textAnchor="middle">剖面長度 {model.length.toFixed(2)} m</text><text x="960" y="55" textAnchor="end">樹高 {treeHeight} m｜枝下高 {trunkHeight} m｜DBH {dbhCm} cm</text></>}
      </g>
      <ViewportExportBounds bounds={viewport.bounds}/><DrawingScale bounds={viewport.bounds} unitsPerSvg={1/scale}/>
    </svg>
    <p style={{padding:"12px 18px",fontSize:14}}>牆體開口依實際高度與位置呈現；深色牆為剖切、淡色牆為背景。深色為剖切樹冠，淡色為箭頭方向 {depth} m 內的背景樹木；地面以 ±0.00 水平面示意。樹冠採直徑 4 m 的整形圓柱體，總樹高 {treeHeight} m、枝下高 {trunkHeight} m、胸徑 DBH {dbhCm} cm，圖面隨設定同步更新。</p>
  </div>;
}
