...
export default function AxonView({walls=[],stage=3,width,height,trees,treeHeight,trunkHeight,dbhCm,faces,colors,planBounds,download}:{walls?:Wall[];stage?:number;width:number;height:number;trees:Point[];treeHeight:number;trunkHeight:number;dbhCm:number;faces:Face[];colors:string[];planBounds:{x:number;y:number;width:number;height:number};download:(svg:SVGSVGElement|null,filename:string)=>void}) {
 ...
 const crownHeight=Math.max(.1,treeHeight-trunkHeight),trunkRadius=dbhCm/200;
 const minX=-cb*(height+5)-3,maxX=ca*(width+5)+3,minY=-treeHeight-3,maxY=sa*(width+5)+sb*(height+5)+3;
 ...
 const wallNodes=walls.flatMap(w=>wallSolids(w).map((solid,i)=>({key:`w${w.id}-${i}`,depth:solid.floor.reduce((sum,p)=>sum+project(p.x,p.y).y,0)/4,node:<AxonWall solid={solid} point={point}/>}))).sort((a,b)=>a.depth-b.depth);
 const treeNodes=trees.map((tree,i)=>({key:`t${i}`,depth:project(tree.x,tree.y).y,node:<g>{cylinder(tree,trunkRadius,0,trunkHeight,'#7c7155','#ac9c79')}{cylinder(tree,2,trunkHeight,treeHeight,'#618c68','#b5d4b1')}</g>})).sort((a,b)=>a.depth-b.depth);
 ...
 {wallNodes.map(item=><g key={item.key}>{item.node}</g>)}
 {treeNodes.map(item=><g key={item.key}>{item.node}</g>)}
 ...
 {(()=>{if(!trees.length)return null;const target=[...trees].sort((a,b)=>point(b.x,b.y).x-point(a.x,a.y).x)[0];const base=point(target.x,target.y),top=point(target.x,target.y,treeHeight),branch=point(target.x,target.y,trunkHeight),x=base.x+2*scale+12;return <g fontFamily="sans-serif" fontSize="15" fill="#263b30" stroke="white" strokeWidth="3" paintOrder="stroke"><path d={`M${base.x} ${base.y}H${x}M${top.x} ${top.y}H${x}M${branch.x} ${branch.y}H${x}`} stroke="#304b3d" strokeWidth="1" strokeDasharray="3 3"/><path d={`M${x} ${base.y}V${top.y}M${x-5} ${base.y}h10M${x-5} ${branch.y}h10M${x-5} ${top.y}h10`} fill="none" stroke="#304b3d"/><text x={x+9} y={(base.y+branch.y)/2+5}>枝下高 {trunkHeight} m</text><text x={x+9} y={(branch.y+top.y)/2+5}>冠高 {crownHeight.toFixed(1)} m</text><text x={x} y={top.y-12} textAnchor="middle">樹高 {treeHeight} m</text></g>;})()}
 </g>}
