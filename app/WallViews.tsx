...
export function AxonWall({solid,point}:{solid:ReturnType<typeof wallSolids>[number];point:(x:number,y:number,z:number)=>P}){
 ...
 return <g stroke="#575047" strokeWidth=".5">{floor.map((p,i)=>{const q=floor[(i+1)%4];return <path key={i} fill={i%2?'#9f9688':'#bbb3a5'} d={path([point(p.x,p.y,z0),point(q.x,q.y,z0),point(q.x,q.y,z1),point(p.x,p.y,z1)])}/>;})}<path fill="#e3dccf" d={path(floor.map(p=>point(p.x,p.y,z1)))}/></g>;
}
